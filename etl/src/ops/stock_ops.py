...
def incert_update_category(cat):
    ...
    category = categories_crud.get_or_create_category(
        db,
        Category(
            store_id=store.id,
            category_name=cat[0],
...
def process_stock_products(prod: str):
    ...
    brand = (
        product.find_all("div", class_="product-brand")[0]
        .text.replace("\n", " ")
        .replace("\r", "")
    )
    ...
    if isinstance(brand, str):
        b = brand_crud.get_or_create_brand(db, Brand_Name(brand_name=brand))
        ...
        p = product_crud.create_product(db, new_product)
